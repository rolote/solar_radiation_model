...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Changing field 'File.order'
        db.alter_column('requester_file', 'order_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['requester.Order'], null=True))

        # Changing field 'File.remotename'
        db.alter_column('requester_file', 'remotename', self.gf('django.db.models.fields.TextField')(null=True))

        # Changing field 'File.size'
        db.alter_column('requester_file', 'size', self.gf('django.db.models.fields.IntegerField')(null=True))

    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'File.order'
        raise RuntimeError("Cannot reverse this migration. 'File.order' and its values cannot be restored.")

        # User chose to not deal with backwards NULL issues for 'File.remotename'
        raise RuntimeError("Cannot reverse this migration. 'File.remotename' and its values cannot be restored.")

        # User chose to not deal with backwards NULL issues for 'File.size'
        raise RuntimeError("Cannot reverse this migration. 'File.size' and its values cannot be restored.")
